...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte beats address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an beats account at a particular block.
    type Account {
    ...
    # Transaction is an beats transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an beats account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an beats account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an beats block by number or by hash. If neither is
