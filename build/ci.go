...
import (
	...
	"github.com/Blockchaindevscol/beatschain/crypto/signify"
	"github.com/Blockchaindevscol/beatschain/internal/build"
	"github.com/Blockchaindevscol/beatschain/params"
	"github.com/cespare/cp"
)

var (
	// Files that end up in the gbeats*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gbeats"),
	}

	// Files that end up in the gbeats-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gbeats     = "gbeats-" + basegeth + ext
		alltools = "gbeats-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gbeats, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gbeats, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gbeats-release.pub"
...
func doDocker(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, gbeatsonly
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, gbeatsonly
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, gbeats& tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, gbeats& tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, gbeatsonly
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, gbeats& tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, gbeatsonly
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, gbeats& tools
	var tags []string
	...
	if len(*manifest) != 0 {
		...
		// Assemble and push the gbeatsmanifest image
		for _, tag := range tags {
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gbeats-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gbeats-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gbeats.exe" {
			gethTool = file
		} else {
	...
	// first section contains the gbeats binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gbeats.nsi", filepath.Join(*workdir, "gbeats.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gbeats-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gbeats.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/Blockchaindevscol/beatschain/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gbeats.aar", filepath.Join(GOBIN, "gbeats.aar"))
		os.Rename("gbeats-sources.jar", filepath.Join(GOBIN, "gbeats-sources.jar"))
		return
	}
	...
	archive := "gbeats-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gbeats.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/Blockchaindevscol/beatschain/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "gbeats-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
