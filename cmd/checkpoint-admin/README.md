...
```shell
go get github.com/Blockchaindevscol/beatschain/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://gbeatsCryptoBeats.org/docs/clef/tutorial) .
...
*It is worth noting that the connected gbeatsnode can be a fullnode or a light client. If it is fullnode, you must enable the LES protocol. E.G. add `--light.serv 50` to the startup command line flags*.
...
*CHECKPOINT_HASH is obtained based on this [calculation method](https://github.com/Blockchaindevscol/beatschain/blob/master/params/config.go#L251).*
...
* Start gbeats with the modified configuration file
