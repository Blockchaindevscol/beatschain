...
Use `devp2p enrdump <base64>` to verify and display an beats Node Record.
...
1. initialize the gbeats node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gbeats with the following flags:
```
gbeats --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gbeats node:
 ```
...
To run the eth66 protocol test suite, initialize a gbeats node as described above and run the following command,
replacing `<enode>` with the enode of the gbeats node:
...
[dns-tutorial]: https://gbeatsCryptoBeats.org/docs/developers/dns-discovery-setup
