...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gbeats is "gbeats". If no
...
func (c *Config) NodeName() string {
	...
	if name == "gbeats" || name == "gbeats-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gbeats" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gbeats 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gbeats' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-beats-keystore")
		isEphemeral = true
	}
